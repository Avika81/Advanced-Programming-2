package com.example.avika.imageservice;
...
public class Client {
    ...
    private static byte[] extractBytes(File file) throws IOException {
        // open image
        System.out.println(file.exists() + "!");  // prints whether the file exists.
        FileInputStream fis = new FileInputStream(file);
        ByteArrayOutputStream bos = new ByteArrayOutputStream();
        byte[] buf = new byte[1024];  //buffer for the picture.
        try {
        ...
        } catch (IOException ex) {  //throw exeption
